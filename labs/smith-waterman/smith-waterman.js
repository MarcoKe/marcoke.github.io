...
function findHighestScore(table) {
    ...
    for (var i = 0; i < table.rows; i++) {
        for (var j = 0; j < table.cols; j++) {
            ...
            else if (table.matrix[i][j] === highestScore) {
                position.push([i, j]);
            }
        }
    }
...
function findAllSolutions(table, seq1, seq2) {
    var start = findHighestScore(table);
    var solutions = [];

    for (var i = 0; i < start.length && i <= 10; i++) {
        solutions.push(traceback(table, seq1, seq2, start[i]));
    }
...
function traceback(table, seq1, seq2, start) {
    var alignment1 = "";
    var alignment2 = "";
    var matches = "";
